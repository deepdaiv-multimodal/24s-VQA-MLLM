...
class VQAHandler(TaskHandler):
    ...
    def train_batch(self, model, image, language_tokens, padding_mask, labels):
        ...
        # print(f'shape of logits: {logits.shape}, labels: {labels.shape}')
        # exit()
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        for tensor_key in data.keys():
            ...
            if loss_scaler is None and tensor_key.startswith("image"):
                data[tensor_key] = data[tensor_key].float()
        ...
        # print('labels:', data['labels'][0]) # torch.Size([bs, 2910])
        # print('labels shape:', data['labels'].shape) # torch.Size([bs, 2910])
