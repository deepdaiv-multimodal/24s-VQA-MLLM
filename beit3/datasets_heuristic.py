...
class OKVQADataset(BaseDataset):
    ...
    def get_index_files(split, task=None):
        ...
        elif split == 'okvqa_train':
            return ("/root/datasets/okvqa/data/okvqa.train.jsonl", )
        elif split == 'aokvqa_train':
...
class CombinedDataset(torch.utils.data.Dataset):
    def __init__(self, datasets):
        ...
        for dataset in datasets:
            if hasattr(dataset, 'label2ans'):
                self.label2ans = dataset.label2ans
                break
...
def create_dataset_by_split(split, is_train=True):
    ...
    # combined_dataset = WrappedConcatDataset([okvqa_dataset, aokvqa_dataset])
    ...
    return create_dataloader(
        ...
        num_workers=32, pin_mem=True, dist_eval=False, 
    )
...
def create_downstream_dataset(is_eval=False):
    if is_eval:
        ...
        return \
            create_dataset_by_split(split="aokvqa_train", is_train=True), \
            create_dataset_by_split(split="okvqa_train", is_train=True)
